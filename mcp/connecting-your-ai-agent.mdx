---
title: Connecting your AI agent
description: How to connect Claude Desktop, Claude Code or any other MCP client to the Cloud 66 MCP server, and what happens during authorization.
lead: Point your MCP client at https://app.cloud66.com/mcp, approve the permissions in your browser, and your agent is connected.
---
...
- [Two-factor authentication](/:product/:version?/account/two-factor-authentication) enabled on your
...
<Callout type="info" title="No client registration needed">
...
<Callout type="warning" title="One active token per client">
...
- If the agent needs to run a destructive tool, it needs an **open MCP session** with elicitation
  ...
  [Approving sensitive actions](/:product/:version?/mcp/approving-sensitive-actions).

If something goes wrong, see [Troubleshooting](/:product/:version?/mcp/troubleshooting).
