---
title: MCP permissions and scopes
description: How Cloud 66 MCP scopes work, what the default permissions are, and how to review or revoke an AI agent's access to your account.
lead: Every MCP connection is limited by the scopes you approve on the consent screen — and by your own role permissions. Both can be reviewed and revoked at any time.
---
...
<Callout type="info" title="Tools are hidden, not just blocked">
...
<Callout type="warning" title="Revoke first, ask later">
...
[account audit log](/:product/:version?/account/audit-logs), along with the IP address the agent
called from.
